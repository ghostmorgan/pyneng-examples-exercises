...
def generate_trunk_config(intf_vlan_mapping, trunk_template):
  """
  Функция, генерирующая шаблон для trunk интерфейсов
  """
  trunk_result_dict = {}

  for intf, vlans in intf_vlan_mapping.items():
    trunk_result_dict[intf] = []
    for command in trunk_template:
      if "allowed" in command:
        trunk_result_dict[intf].append(f"{command} {','.join([str(vlan) for vlan in vlans])}")
      else:
        trunk_result_dict[intf].append(command)
  
  return trunk_result_dict
